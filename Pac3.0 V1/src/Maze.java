...
// 2d character array that stores the maze display
	...
	// constructor which generates the maze
	// store an 'X'for blocks and 'O' for walkways in the maze.
	public Maze()
	{
		try 
		{
	...
	/**
	 * creates a 2D array from  datafile provided to represent the maze
	...
	/**
	 * generates graphics to display the path from the player to the exit of the maze
	 */
	private void drawPath()
	{
		for (int r = 0; r < numRow; r++)
		{
			for (int c = 0; c < numCol; c++)
			{
				if(mat[r][c] == '.')
				{
					Sprite2D s = new Sprite2D("CameraExtend",new Vector3f(0,0,0),new Vector3f(1,1,1));
					blockWidth = s.getTexture().getDimensions().x;
					blockHeight = s.getTexture().getDimensions().y;
	...
	/**
	 * Deletes the modifications to the 2D array made by drawPath() 
	 */
	...
	//NOT IN USE
	/**
	 * solves the path from the player to the exit of the maze
	 */
	...
	
	{
      ...
      while(true) 
      {
    	  System.out.println(currentMove.rPos + " " + currentMove.cPos + " ");
	...
	/**
	 * generate a path from a starting location to the maze exit and displays the graphics
	 * @param currentRow row to be used as the start location of the path
	 * @param currentCol column to be used as the start location of the path
	 */
	public void displayPath(int currentRow, int currentCol)
	{
		startCol = currentCol;
		startRow = currentRow;
		currentMove = new Coord(startRow,startCol);
		...
		drawPath();
		deletePath();
		System.out.println("Finished displaypath");
	}
	
	public void displayArrow(int playerRow, int playerCol)
	{
		...
		Vector3f startPos = new Vector3f(0,0,0);
		Vector3f currentPos = new Vector3f(0,0,0);
		currentPos.x = startPos.x+blockWidth*playerCol;
		currentPos.y = startPos.y-blockHeight*playerRow;
		...
		//System.out.println("expression : " + (Math.abs(playerCol-endCol))/(Math.abs(playerRow-endRow)));
		//System.out.println("abs : " + (Math.abs(playerCol-endCol)));
	...
	// This method checks four possible positions in a counter-clock wise manner
